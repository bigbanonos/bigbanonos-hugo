...
def parse_name(stem):
    # Songs-Aa-2020s -> letter='A', era='2020s'
    m = re.match(r'Songs-([A-Za-z#]+)-(.+)', stem)
    if not m: return None, None
    L = m.group(1)[:1].upper()
    if L == '#' or not L.isalpha(): L = '#'
...
def make_post(csv_path, out_dir):
    ...
    if not letter or era == 'All': return None  # skip -All files
    ...
    years = [yr(r.get('Release Date','')) for r in rows]
...
if __name__ == '__main__':
    for f in sys.argv[1:]:
        r = make_post(f, '/tmp/oneoff_stubs')
        if r: print(f'\u2713 {r.name}')
        else: print(f'\u2717 skipped {Path(f).name}')
